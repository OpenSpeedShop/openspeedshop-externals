...
BASE_DIR=/usr/cta/pet/pkgs/openss
TOOL_VERS="_v2.4.1.release"
ROOT_VERS="_v2.4.1.release"
GRAPHVIZ_VERS="-2.40.1"
QTGRAPH_VERS="-1.0.0"

MPT_IDIR=/p/app/sgi/mpt/mpt-2.15
MPICH_IDIR=/p/app/intel/parallel_studio_xe_2019_update1/impi/2019.1.144/intel64
...
./install-tool --verbose --build-cbtf-all --cbtf-prefix ${CBTF_IDIR} --krell-root-prefix ${KROOT_IDIR} --with-mpich ${MPICH_IDIR} --with-mpt ${MPT_IDIR} --with-ltdl ${LTDL_IDIR} --with-python ${PYTHON_IDIR} --with-python-vers 2.7  2>&1 | tee install_tool_build_cbtf.log

# Build openspeedshop using cbtf components and the root external packages
./install-tool --verbose --build-oss --openss-prefix ${OSSCBTF_IDIR} --cbtf-prefix ${CBTF_IDIR} --krell-root-prefix ${KROOT_IDIR} --with-python ${PYTHON_IDIR} --with-python-vers 2.7 --with-mpich ${MPICH_IDIR} --with-mpt ${MPT_IDIR} --with-ltdl ${LTDL_IDIR} 2>&1 | tee install_tool_build_oss.log
