...
MPT_IDIR=/p/app/hpe/mpt-2.17
MPICH_IDIR=/p/app/intel/parallel_studio_xe_2018_update1/impi/2018.1.163/intel64
...
./install-tool --verbose --build-krell-root --krell-root-prefix ${KROOT_IDIR} --with-mpich ${MPICH_IDIR} --with-mpt ${MPT_IDIR} --with-ltdl ${LTDL_IDIR} 2>&1 | tee install_tool_build_krell_root.log

# Build cbtf (base cbtf support) , cbtf-krell (collectors, runtimes), cbtf-argonavis (gpu support), cbtf-lanl (sys admin tools)
./install-tool --verbose --build-cbtf-all --cbtf-prefix ${CBTF_IDIR} --krell-root-prefix ${KROOT_IDIR} --with-mpich ${MPICH_IDIR} --with-mpt ${MPT_IDIR} --with-ltdl ${LTDL_IDIR}  --with-cuda ${CUDA_IDIR} --with-cupti ${CUPTI_IDIR} --with-python ${PYTHON_IDIR} --with-python-vers ${PYTHON_VERS} 2>&1 | tee install_tool_build_cbtf.log
...
./install-tool --verbose --build-oss --openss-prefix ${OSSCBTF_IDIR} --cbtf-prefix ${CBTF_IDIR} --krell-root-prefix ${KROOT_IDIR} --with-mpich ${MPICH_IDIR} --with-mpt ${MPT_IDIR} --with-ltdl ${LTDL_IDIR} --with-cuda ${CUDA_IDIR} --with-cupti ${CUPTI_IDIR} --with-python ${PYTHON_IDIR} --with-python-vers ${PYTHON_VERS} 2>&1 | tee install_tool_build_oss.log
